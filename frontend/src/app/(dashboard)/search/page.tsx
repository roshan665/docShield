...
export default function SearchPage() {
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      {/* Main Search Input & Reference Filter Chips */}
      <div className="space-y-4">
        <form onSubmit={handleExecuteSearch} className="flex gap-2">
          <div className="relative flex-1">
            <Search className="absolute left-3.5 top-3.5 h-4 w-4 text-slate-400" />
            <Input
              placeholder="Search keywords, people, cases..."
              value={query}
              onChange={(e) => setQuery(e.target.value)}
              className="pl-10 h-11 rounded-xl bg-white border-slate-200 text-xs shadow-sm"
            />
          </div>
          <Button
            type="submit"
            disabled={loadingSearch || !query.trim()}
            className="h-11 px-5 rounded-xl bg-blue-600 hover:bg-blue-500 text-white text-xs font-semibold gap-2 shadow-md shadow-blue-600/20 shrink-0"
          >
            {loadingSearch ? (
        ...
        {/* Filter Chips (Reference Panel 7) */}
        <div className="flex items-center gap-2 overflow-x-auto pb-1 no-scrollbar text-xs">
          {[
            { id: "all", label: "All", mode: "semantic", type: "" },
            { id: "people", label: "People", mode: "keyword", type: "witness_statement" },
            { id: "cases", label: "Cases", mode: "hybrid", type: "fir" },
            { id: "documents", label: "Documents", mode: "semantic", type: "" },
            { id: "rag", label: "Ask AI", mode: "rag", type: "" },
          ].map((chip) => {
            const isSelected =
              chip.id === "rag" ? activeTab === "rag" : activeTab === chip.mode && selectedDocType === chip.type;
            return (
              <button
                key={chip.id}
                onClick={() => {
                  if (chip.id === "rag") {
                    ...
                    setActiveTab(chip.mode as SearchMode);
                    setSelectedDocType(chip.type);
                  }
                }}
                className={cn(
                  "px-4 py-1.5 rounded-full font-medium whitespace-nowrap transition-all text-xs",
                  isSelected
      ...
      {/* QUICK SEARCH & RECENT SEARCHES (Displayed when no active results or as shortcuts) */}
      {!searchResults && activeTab !== "rag" && (
        <div className="space-y-6">
          {/* Quick Search 2x2 Grid (Reference Panel 7) */}
          <div className="space-y-2.5">
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                {
                  label: "Face Search",
                  desc: "Biometric & facial matches",
                  icon: Sparkles,
                  query: "face identification suspect photo biometric match",
                  iconColor: "bg-blue-50 text-blue-600 border-blue-100",
                },
                {
                  label: "Document Search",
                  desc: "Legal filings & affidavits",
                  icon: FileText,
                  query: "forensic report chargesheet affidavit",
                  iconColor: "bg-blue-50 text-blue-600 border-blue-100",
                },
                {
                  label: "Case Search",
                  desc: "FIR & investigation briefs",
                  icon: Search,
                  query: "FIR cyber crime financial fraud case",
                  iconColor: "bg-emerald-50 text-emerald-600 border-emerald-100",
                },
                {
                  label: "Location Search",
                  desc: "Incident spots & crime scene",
                  icon: FolderLock,
                  query: "crime scene location seizure panchnama site",
                ...
                <div
                  ...
                  className="flex items-center gap-3 p-3.5 rounded-2xl bg-white border border-slate-200/90 hover:border-blue-400 hover:shadow-md transition-all cursor-pointer shadow-sm group"
                >
                  <div className={cn("p-2.5 rounded-xl border shrink-0", item.iconColor)}>
          ...
          {/* Recent Searches (Reference Panel 7) */}
          <div className="space-y-2.5">
            <h3 className="text-xs font-bold text-slate-900 uppercase tracking-wider">
              Recent Searches
            </h3>
            <div className="space-y-2">
              {[
                { term: "NCIB-2024-001", tag: "Case ID" },
                { term: "Amit Sharma", tag: "Person" },
                { term: "Forensic Report", tag: "Document" },
                { term: "Bhopal", tag: "Location" },
              ].map((item) => (
                <div
                  key={item.term}
                  onClick={() => {
                  ...
                  className="flex items-center justify-between p-3 rounded-2xl bg-white border border-slate-200/90 hover:border-blue-400 hover:shadow-sm transition-all cursor-pointer group"
                >
                  ...
                  <ChevronRight className="h-4 w-4 text-slate-300 group-hover:text-blue-600 transition-transform group-hover:translate-x-0.5" />
                </div>
              ))}
            </div>
          </div>
