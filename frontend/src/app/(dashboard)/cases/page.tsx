...
export default function CasesPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Search & Filter Toolbar with Reference Filter Chips */}
      <div className="space-y-3">
        <div className="flex items-center gap-2.5">
          <div className="relative flex-1">
            <Search className="absolute left-3.5 top-3 h-4 w-4 text-slate-400" />
            <Input
              placeholder="Search cases by number, title, FIR..."
              value={searchQuery}
        ...
        <div className="flex items-center gap-2 overflow-x-auto pb-1 no-scrollbar text-xs">
          {[
            ...
            { id: "pending_legal", label: "Pending Legal" },
      ...
      {/* Case Records List / Cards (Matching Reference Panel 4) */}
      {loading ? (
        ...
        <div className="space-y-3 sm:grid sm:grid-cols-2 lg:grid-cols-3 sm:space-y-0 sm:gap-4">
          {cases.map((c, index) => {
            // Pick a soft accent color for card icon container
            const iconBg =
              c.priority === "critical"
            ...
            return (
              <div
                ...
                className="group flex items-center justify-between p-4 rounded-2xl bg-white border border-slate-200/90 hover:border-blue-400 hover:shadow-md transition-all cursor-pointer shadow-sm"
              >
                <div className="flex items-start gap-3.5 min-w-0 flex-1">
                  {/* Icon Container */}
                  <div className={cn("p-2.5 rounded-xl border shrink-0 mt-0.5", iconBg)}>
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      <span className="font-bold text-xs sm:text-sm text-slate-900 font-sans truncate">
                        Case #{c.case_number}
                      </span>
                    </div>
                      ...
                      <span
                        className={cn(
                          ...
                          c.status === "open"
                            ...
                            : c.status === "pending_review" || c.status === "pending_legal"
                ...
                {/* Right Arrow */}
                <ChevronRight className="h-5 w-5 text-slate-300 group-hover:text-blue-600 transition-transform group-hover:translate-x-1 shrink-0 ml-2" />
              </div>
            );
          })}
        </div>
